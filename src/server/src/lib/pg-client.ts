...
class PgClient {
    ...
    async executeReadQuery(value: ClientValue): Promise<ExecuteResult<ClientResult>> {
    	...
    	if (!replicaPoolInfo || !replicaPoolInfo.pool) {
    		throw new PgClientError(
    			DbErrorType.NO_POOL,
    			`There isn't an active pool for executeReadQuery: ${value}`,
    		);
    	}
    	const {pool} = replicaPoolInfo;
    	const result = await execute<ClientResult>(
    		async (client: pg.PoolClient) => client.query(value),
    		pool,
    	);
    	return result;
    }
    ...
    async executeWriteQuery(value: ClientValue): Promise<ExecuteResult<ClientResult>> {
    	...
    	if (!masterPoolInfo || !masterPoolInfo.pool) {
    		throw new PgClientError(
    			DbErrorType.NO_POOL,
    			`There isn't an active pool for executeWriteQuery: ${value}`,
    		);
    	}
    ...
    async _assertPoolManagerIsActive(): Promise<void> {
    	...
    	if (!this._isPoolsManagerActive) {
    		throw new PgClientError(
    			DbErrorType.POOL_ERROR,
    			'Pool manager is inactive. Cannot provide database client',
    		);
    	}
    }
    ...
    private _managePool(poolInfo: PoolInfo): Promise<void> {
    	return new Promise((resolve, reject) => {
    		...
    		execute<ClientResult>(
    			async (client: pg.PoolClient) => client.query(QUERY_INFO_REPLICA),
                poolInfo.pool!,
    		)
