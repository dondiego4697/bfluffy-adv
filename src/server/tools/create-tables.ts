...
const queryAlembicVersion = knex.select({version: 'version_num'}).from(TABLE_ALEMBIC_VERSION);
...
async function insertRows(queries: string[], version: Version) {
    await dbManager.executeInTransaction(async (client) => {
        ...
        if (version.current) {
            ...
            const query = knex(TABLE_ALEMBIC_VERSION).insert({version_num: version.next});
