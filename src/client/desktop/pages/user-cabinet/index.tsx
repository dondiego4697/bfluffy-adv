...
import './index.scss';
import { AdListItem } from 'client/components/ad-list-item';
        ...
        return (
            <div className={b('my-ads-panel')}>
                {this.state.ads.map((item, index) => (
                    <AdListItem
                        key={`user-cabinet-my-ad-${index}`}
                        breed={item.animalBreedDisplayName}
                        ...
                        onClickHandler={() => this.props.history.push(RoutePaths.AD_EDIT.replace(':id', item.publicId))}
                    />
                ))}
            </div>
        );
