...
export class AnimalModel {
	...
	@action public init() {
    	return AnimalRequestBookV1.getBreedList()
    		.then((response) => {
	...
	public findBreedByName(params: FindBreedByNameParams): AnimalBreed[] {
		...
		if (!categoryCode && subtext) {
			return this.breedList.filter(
				(item) => item.breedDisplayName.toLowerCase().includes(subtext.toLowerCase())
			);
		}

		if (categoryCode && subtext) {
			return (this.breedHash[categoryCode] || []).filter(
				(item) => item.breedDisplayName.toLowerCase().includes(subtext.toLowerCase())
			);
		}
