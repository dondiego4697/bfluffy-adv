...
import {FarmRequestBookV1, FarmInfo, CreateFarmParams} from 'client/lib/request-book/v1/farm';
...
export class AdEditPageModel extends BasePageModel {
    ...
    @action public createFarm(farm: CreateFarmParams) {
        ...
        return FarmRequestBookV1.createFarm(farm).finally(() => this.setReady());
    }

    @action public updateFarm(farm: CreateFarmParams) {
        this.setLoading();

        return FarmRequestBookV1.updateFarm(this.farm!.farmPublicId, farm).finally(() => this.setReady());
    }
